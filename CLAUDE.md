...
- `20260520_meal_plans_payment_qr_gst.sql` — adds `properties.meal_plans` (jsonb), `properties.payment_qr_data_url`/`payment_qr_label` (text), and `room_categories.gst_rate` (smallint). Fixes a latent cloud round-trip bug where these UI-side fields were silently dropped on save. **Paste into Supabase SQL Editor before flipping DEMO_MODE off.**
