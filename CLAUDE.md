...
- **Hold management (Q3) + ticker hoist (#8).** A real feature across client + server:
  ...
  - **OTA inventory on hold create/release** already rides the existing AIOSELL auto-sync (booking `status` is in its signature); the server-side auto-release reconciles OTAs on the next app-open.
