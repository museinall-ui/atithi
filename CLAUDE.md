...
**AIOSELL channel-manager partnership (in progress, owner-facing).** AIOSELL is in the same group as STAAH. Reseller agreement reviewed clause-by-clause (no unusual terms flagged), **signed by the owner**, and the partner reply + webhook URL sent. Hotel-API pricing confirmed (~₹800/mo base for 0–10 rooms; reseller discount tiers 10/15/20%). The four required CM flows (inventory push, rate push, reservation webhook, cancellation sync) map cleanly to AtithiBook's shapes. **Next, when AIOSELL returns API credentials (~15-min go-live):** set the AIOSELL env vars in Vercel + the webhook Basic-auth password + map the hotel in their Operator Console. "VR API" = their Vacation-Rental API variant — not what we need; we use the hotel `cm` API.
