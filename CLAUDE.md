...
- **Google OAuth — wired (early July 2026)**: `signInWithGoogle()` helper in `src/supabase.js`, "Continue with Google" button at the top of `src/screens/SignIn.jsx` (with the official 4-colour Google G drawn inline). **LIVE + working (Jul 2026)** — owner created the Google Cloud OAuth client (id `262648759612-…apps.googleusercontent.com`, redirect URI `https://vaerzwmglfwslvqqcyhx.supabase.co/auth/v1/callback`), published the consent screen, and enabled Google in Supabase → Providers; sign-in tested end-to-end. The friendly setup hint (`googleNotEnabled` / `googleSignInError`) stays as a fallback. **Identity note:** Google + magic-link auto-merge into ONE account when they share a **verified email** (both verify it); a *different* email = a separate account (separate hotel/data) — tell hoteliers to use the same email every time.
...
- `20260629_hold_reminder.sql` — adds `bookings.hold_reminder_sent_at` (timestamptz). De-dupes the hold-expiry phone reminder sent by the `api/hold-watch` cron (once per booking via an atomic NULL→now claim). No RLS change — hold-watch runs with the service-role key and the column is never read by the browser. Idempotent. **Paste before turning on phone hold-reminders.**
- `20260630_storage_buckets.sql` — **Phase 4 storage migration.** Creates two Supabase Storage buckets so images + voice notes stop living as base64 blobs in DB rows: `property-media` (PUBLIC, 5MB, images — logo/payment-QR/room-photos/gallery) + `property-audio` (PRIVATE, 10MB, voice notes), both with member-scoped RLS via `has_property_access(((storage.foldername(name))[1])::uuid)` (path = `<propertyId>/<file>`). New uploads go to Storage (CDN URL) via `src/cloud/storage.js`; `<img>/<audio>` render URL + base64 identically so old data keeps working; a one-time auto-backfill (App.jsx) moves existing base64 images over. Voucher auto-print now waits for images to load (URLs fetch at print time). **Paste to enable Storage uploads — until then uploads silently keep using base64 (no breakage).** Idempotent.

- `20260701_storage_rls_permissions.sql` — **RBAC fix (round-8 audit).** The 20260630 bucket policies gated property-media writes (+ property-audio read/write) on bare membership, so a low-trust member (reception, no `manage_settings`) could deface/delete the hotel's public logo/QR/room-photos via the Storage API directly even though the UI + properties table block them. This gates property-media insert/update/delete on `has_perm(manage_settings)` and property-audio writes on `edit_bookings` (read stays member-scoped), aligning storage.objects with the per-table RBAC from 20260611. Owner short-circuit keeps owners full-access; public image read unchanged. Idempotent. **Paste before adding reception/manager staff.**

- `20260702_widget_captcha_lockdown.sql` — **CAPTCHA lockdown (Jul 2026, pasted + verified).** Revokes the public/anon direct-insert bypass on the booking widget so the CAPTCHA verifier (`api/widget-book.js`, service-role) is the ONLY path to create a website booking. Must `revoke execute ... from public` (not just anon — PUBLIC is the default grant). Revert block re-opens the old path. Idempotent. **Paste AFTER `TURNSTILE_SECRET_KEY` is set + a real test booking works.**
- `20260703_team_member_emails.sql` — **Team emails (Jul 2026, pasted).** Security-definer `property_members(p_property_id)` RPC returns each membership row + the member's email (auth.users is RLS-blocked), gated by `has_property_access`. `src/cloud/team.js` `loadMembers` uses it with a graceful fallback to the raw id. Idempotent. **Paste before adding staff** so Settings → Team shows emails, not codes.
...
### July 2026 — CAPTCHA live + Google sign-in + owner setup + final code & hotelier audits

A long session that (a) took the public booking link live-safe, (b) finished the owner-side setup, and (c) ran two final multi-agent audits and fixed almost everything. All shipped to `main`.

- **CAPTCHA shipped** — see the "CAPTCHA" bullet in Open/queued (now DONE) + migrations `20260702` / self-locked `20260628`. New file `api/widget-book.js`.
- **Owner setup completed + verified** (walked step-by-step, all probed live): Cloudflare Turnstile account + `TURNSTILE_SECRET_KEY` in Vercel; **Google sign-in** live (see the Auth bullet); Supabase Auth Site/Redirect URLs confirmed; all migrations current (incl. `20260702`, `20260703`); **`VITE_CONTACT_WA` = `919677184198`** set. Owner-facing step-by-step lives in **[OWNER_NEXT_STEPS.md](./OWNER_NEXT_STEPS.md)**.
- **Final code-end audit (2026-06-30)** — 10-dimension multi-agent finders + adversarial verify. 6 confirmed (0 critical/high): the CAPTCHA lockdown self-revert guard (self-locking `20260628`), ANCHOR going stale at IST midnight on an always-on tab (30s-ticker reload guard in `App.jsx`), a Turnstile script-load dead-end (recoverable message), a client-total clamp in `api/widget-book.js`, a saved-extra false-error toast (ref advance), and a role-aware cloud-load-error fallback. All fixed + shipped. Report: **[CODE_AUDIT_2026-06-30.md](./CODE_AUDIT_2026-06-30.md)**.
- **Final hotelier-end (UX) audit (2026-06-30)** — 12 journeys walked as a non-technical owner + adversarial verify. 40 confirmed → deduped to 35 (5 high, ~17 med, ~13 low). **~30 fixed + shipped in 6 reviewable batches** (guest-facing money wording · Reports/Dashboard numbers · credit/refund clarity · team/RBAC incl. the `20260703` member-email RPC · rates/pricing legibility · diary/first-run polish). **5 deferred with reasons:** M7 + L13 (Rates-tools + widget Hindi → the known-deferred Settings/units i18n pass), M16 (Step-2 custom child-band itemisation — the total charged is already correct; needs a band-driven editor rework), M8-partial (BookingDetail read-only stay dates), L12-partial (a few scattered Hindi words). Full tracker + status: **[AUDIT_HOTELIER_2026-06-30.md](./AUDIT_HOTELIER_2026-06-30.md)**.

**Open / queued:**
- **CAPTCHA (Cloudflare Turnstile)** on the public booking link — ✅ **DONE + live (Jul 2026).** `api/widget-book.js` verifies the Turnstile token with Cloudflare server-side, then inserts via `book_widget_slot` using the service-role key; the widget renders the "I'm human" check (public site key `0x4AAAAAADsz85O7wFnAki_z` in `App.jsx`, secret in Vercel env `TURNSTILE_SECRET_KEY`). Falls back to the direct insert until the secret is set, so nothing breaks mid-setup. Migration `20260702_widget_captcha_lockdown.sql` (pasted) revokes the anon direct-RPC bypass — **and must revoke from `public`, not just `anon`** (Postgres grants functions to PUBLIC by default, so revoking anon alone was a no-op; probe-verified). `20260628` was also made self-locking so re-pasting a widget migration can't silently re-open it. Verified live (bad token → `captcha_failed`; anon RPC → `permission denied`). Safe to share.
...
- **WhatsApp contact number** — ✅ **SET (Jul 2026).** `VITE_CONTACT_WA` = `919677184198` in Vercel (91 + the 10 digits; verified baked into the live bundle). Powers the Landing demo-gate WhatsApp button + the Channels support button.
...
### Late June 2026 — discuss-item build (holds + day-close + reports + widget children)

Closed out the last 8 "discuss" items from the deep audit (owner: "cover absolutely everything"). All verified in browser preview, no console errors. Commits 159b6bb / b66deca / 409ccb7 / 7244282 / 90b586b.

- **Reports invoice range (#5).** "Invoices issued" count + the downloadable invoice CSV now honor the picked date range (`issuedInvoicesInRange` / `caRegisterInRange` via an `invInRange` date filter), matching their sibling reports. The month-end **Send-to-CA register stays all-time / FY-scoped** — a CA wants the full gap-free sequence, not a slice.
- **Reports inverted range (#6).** The From/To date pickers carry `min`/`max` so an end-before-start range is unreachable in normal use; a `rangeValid` backstop hides the headline KPI grid + P&L card (which would otherwise show clamped 1-day numbers) and shows only the warning.
- **Widget children bands (Q4).** The public widget now collects the same free / half / full age bands reception uses — thresholds from `accountant.childFreeBelowAge` / `childAgeBelow` — but ONLY when a room category (or season) actually charges for extra children (`chargesForChildren`); otherwise a single "Children (total)" stepper keeps the widget simple (owner's no-learning-curve rule). `roomItems` carry `childrenFree` / `children` / `childrenFull`; `totalChildren` feeds the meals + per-guest-extras headcount.
- **Day-close reconciliation (Q2).** The end-of-day close compared the counted drawer against the **sum of today's arrival totals** (wrong — ignored actual collection, double-counted earlier-paid advances, never subtracted expenses). Now the target is the **net cash from today's activity**: money collected today − money spent today, split cash-vs-digital (payments/expenses only reliably distinguish cash from non-cash). Dashboard computes `dayCloseExpected` (needs the new `expenses` prop) and the card shows "Expected today", an "Expected · Cash · Digital" split, and an over-expected / short reconciliation on the closed view. Old close records (no `hadMovement` flag) never show a misleading gap.
- **Hold management (Q3) + ticker hoist (#8).** A real feature across client + server:
  - **Configurable hold window** — `accountant.holdHours` (default 12), replacing the fixed 12h/4h split. The widget's `computeHoldHours` reads it and caps at the guest's check-in time so a hold never runs past arrival.
  - **Auto-release vs reminder** — `accountant.holdMode` ('auto' | 'reminder'), a radio in Settings → Booking link → **Hold & auto-release**. Auto (default) = the unit frees itself + OTAs update when the timer runs out; reminder = the unit stays held until the hotelier decides.
  - **Ticker rework + #8** — the mount-once auto-release ticker now respects `holdMode` (reminder = no destructive cancel) and computes the released-set from a live `bookingsRef.current` **before** `setBookings` (the old code populated an outer array from inside the updater — fragile under double-invoked updaters). Added `propertyRef` / `bookingsRef` mirrors. `paid → confirmed` already lives in `addPayment`.
  - **Reminder-mode nudge** — Dashboard surfaces an "expired hold · extend or release" nudge (in auto mode these are released within ~30s, so the list is empty there).
  - **Phone-when-closed (server)** — `api/hold-watch.js`: a serverless function that buzzes the hotelier's phone (Web Push) when an unpaid hold is within 30 min of expiring, and in auto mode **releases expired holds server-side** (status + the same activity event the ticker writes) so the unit/OTAs free with nobody watching. Reminders fire at most once per booking via the `bookings.hold_reminder_sent_at` claim (migration `20260629`). Dormant (503) until `CRON_SECRET` + the existing VAPID/service-role envs are set; no-ops gracefully before the migration. Driven by `.github/workflows/hold-watch.yml` — a **free** public-repo GitHub Actions schedule every ~15 min (Vercel Hobby cron only runs once/day). **Owner setup: paste 20260629; add `CRON_SECRET` to Vercel env + the same value as a GitHub repo secret; redeploy.**
  - **OTA inventory on hold create/release** rides the existing AIOSELL auto-sync (booking `status` is in its signature) **and** the new server-side push (see the OTA-server-push section below) — so when a hold auto-releases server-side, `api/hold-watch.js` calls `pushInventoryForProperty()` straight after, freeing the unit on the OTAs without waiting for the next app-open.

### Late June 2026 — OTA server-side inventory push + Phase-4 storage move + AIOSELL partnership + round-7/8 audits

A long multi-session sweep. Everything below is shipped to `main` and auto-deployed; builds pass and the live site is healthy. Memory files carry the deep detail: [app-deep-audit-2026-06-24](memory), [aiosell-integration](memory), [photo-storage-move](memory).

**OTA server-side inventory push (the "inventory must reach OTAs even when the app is closed" build).** Inventory is the highest-stakes update for a hotel, so it can no longer depend on a browser being open.
- `lib/aiosellServer.js` — shared server helper: `forwardToAiosell()`, `buildInventoryPayload()`, `pushInventoryForProperty(propertyId)` (recomputes availability per room type for the horizon and pushes to AIOSELL `cm` API), `listConfiguredPropertyIds()`.
- Four trigger paths now push inventory server-side: (1) a new website/staff booking via `api/notify-booking.js` (gated on the claim-won, so only the winning insert pushes), (2) a server-side hold auto-release via `api/hold-watch.js`, (3) a periodic reconciliation sweep `api/ota-inventory-sync.js` (BUDGET_MS-bounded, walks every configured property), (4) the existing in-app AIOSELL auto-sync on the hotelier's device (unchanged, now a belt-and-braces third layer).
- `.github/workflows/hold-watch.yml` pings `hold-watch` then `ota-inventory-sync` every ~15 min (free public-repo GitHub Actions; Vercel Hobby cron only runs once/day). All endpoints are constant-time `CRON_SECRET`-gated and dormant (503) until envs are set.
- **Inbound** reservations from OTAs land via `api/aiosell-reservation.js` (book / modify / cancel keyed by `action`; modify = full replace). Webhook URL given to AIOSELL: `https://www.atithibook.com/api/aiosell-reservation`.

**Phase 4 — storage move (DONE).** Images + voice notes moved from base64-in-DB-rows to Supabase Storage. `src/cloud/storage.js` (`uploadPropertyMedia` / `uploadPropertyAudio` / `migratePropertyImages` / `deletePropertyMedia` / `signedAudioUrl` …). Public `property-media` bucket (CDN URLs for logo/QR/room-photos/gallery) + private `property-audio` (signed URLs for voice notes). `<img>`/`<audio>` render a Storage URL and an old base64 blob identically, so existing data keeps working; a one-time per-session auto-backfill in App.jsx (base64-guarded per field) lifts existing images into Storage. Old objects are deleted on photo remove/replace + category delete. Voucher auto-print waits for image loads. Migrations `20260630_storage_buckets.sql` (buckets) + `20260701_storage_rls_permissions.sql` (round-8 RBAC — media writes need `manage_settings`, audio writes need `edit_bookings`). **Owner: both pasted + verified live.** This supersedes the old "Phase 4 — TBD" roadmap line below.

**AIOSELL channel-manager partnership — CREDS RECEIVED + ENV VARS SET, awaiting a pilot property.** AIOSELL is in the same group as STAAH. Reseller agreement reviewed clause-by-clause (no unusual terms flagged) + **signed by the owner**. Hotel-API pricing: ~₹800/mo base for 0–10 rooms; reseller discount tiers 10/15/20%. The four CM flows (inventory push, rate push, reservation webhook, cancellation sync) map cleanly to AtithiBook's shapes. "VR API" = their Vacation-Rental variant — NOT what we need; we use the hotel `cm` API.

**What's DONE (2026-06-26):** AIOSELL added "Atithi Book" as a PMS partner and issued live push details. **Partner slug = `atithi-book`.** The push URLs they gave exactly match what the code generates from that slug (base default `https://live.aiosell.com/api/v2/cm` unchanged) — inventory `…/update/atithi-book`, rates `…/update-rates/atithi-book` — so **no code change was needed** and `AIOSELL_BASE_URL` is deliberately left UNSET. All **5 Vercel env vars are set** by the owner: `AIOSELL_USERNAME` (=`atithi-book`), `AIOSELL_PASSWORD` (secret — Vercel only, never in repo), `AIOSELL_PMS_SLUG` (=`atithi-book`), `AIOSELL_WEBHOOK_USER` (=`atithibook`), `AIOSELL_WEBHOOK_PASS` (secret — Vercel only). The inbound webhook **fails closed** (503) until the last two are set; constant-time compare. Migrations `20260622_aiosell_reservation_ids.sql` + `20260624_aiosell_ext_ota_unique.sql` **pasted + verified** (probed live: `bookings.ext_ota_id` + `ext_channel` exist). AIOSELL also sent a property-onboarding form: `https://zfrmz.com/XrAlMhLuFnuKDcitHz0G`.

**BLOCKER — no pilot property right now.** **Yatra Desert Camp (the demo/seed property) is CLOSED as a real business**, so there is no live hotel to onboard yet. The owner will source a first property and say when ready (declined help drafting outreach). The integration is therefore **fully staged + dormant-safe** — nothing fires without a mapped property.

**REMAINING GO-LIVE SEQUENCE (one ~20-min pass the day a property lands):** (1) **redeploy** so the env vars activate (Vercel env changes need a fresh deploy — offer to push a tiny commit); (2) fill the AIOSELL **onboarding form** with the real property (it likely asks for the reservation-webhook URL + Basic-auth → give `https://www.atithibook.com/api/aiosell-reservation` + the `AIOSELL_WEBHOOK_USER`/`PASS` values); (3) once AIOSELL assigns the **hotelCode + room/rateplan codes**, map them in the **Operator Console** (admin-only screen, owner's email; writes `accountant.aiosell`); (4) **verify** a test push reaches AIOSELL + a Webhook-Tester reservation lands in the diary. Until a property exists, there is no more setup to do. Deep detail in memory [[aiosell-integration]].

**Round-7 verify + round-8 deep audit (both done).** Two full multi-agent audit passes (round-7: verify-end-to-end of the storage + OTA + holds work; round-8: a deeper sweep that specifically regression-checked round-7's own fixes). Round-8 caught and fixed a **critical self-introduced regression**: the round-7 auto-release ticker pushed `appliedIds` *inside* a `setBookings` functional updater then read it synchronously — React updaters aren't synchronous, so cloud-sync / activity-log / undo never fired for auto-releases. Fixed by using the `changed` set directly + a conditional `releaseHoldCloud()` (cloud cancel guarded on `status='tentative' AND release_ts<=now` so two devices can't double-cancel). Other round-8 fixes: `computeUnitUsage` now honours per-night `nightTypes` for inventory; Dashboard day-close uses the live clock (`ymd(new Date())`) not ANCHOR + coherent counted-vs-expected derivation on the closed view; widget-booking edit no longer double-applies the rate-plan multiplier (saved `roomItems` carry `rate:null`); storage RBAC (20260701). **Deferred (not yet built):** a two-device "version check" on the property full-row save to avoid last-write-wins clobber — low risk until real team members exist; build when staff are added.

**Owner setup / accounts done this session:** `trishi@atithibook.com` live on Zoho Mail (MX/SPF/DKIM/DMARC verified externally); Udyam/MSME registration in progress (free govt site, satisfies the "GST not required" path for WhatsApp later).
...
### Phase 4 — Storage (Supabase Storage for logos + room photos) — ✅ DONE (late June 2026)
Images + voice notes moved from base64-in-DB to Supabase Storage (public `property-media` + private `property-audio`) via `src/cloud/storage.js`, with a one-time auto-backfill and forward-compatible URL/base64 rendering. Migrations `20260630_storage_buckets.sql` + `20260701_storage_rls_permissions.sql` (both pasted + verified). See the "OTA server-side inventory push + Phase-4 storage move" narrative section above for the full shape.

### Phase 5 — OTA channel manager — IN PROGRESS (partner = AIOSELL, agreement signed)
**Update (late June 2026): the chosen partner is AIOSELL** (same group as STAAH), not STAAH directly. Reseller agreement signed; server-side push/inbound code is already built (`lib/aiosellServer.js`, `api/aiosell-reservation.js`, `api/ota-inventory-sync.js`). Awaiting AIOSELL API credentials for the ~15-min go-live. See the "OTA server-side inventory push + AIOSELL partnership" narrative section above. The STAAH research below is kept for history / fallback.
