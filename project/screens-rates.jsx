// screens-rates.jsx — Rates & inventory with calendar grid, bulk edit, discounts

function Rates({ go, t, lang }) {
  const [selectedType, setSelectedType] = React.useState('dlx');
  const [selected, setSelected] = React.useState(new Set([2, 3])); // selected day indices
  const [dragStart, setDragStart] = React.useState(null);
  const [dragEnd, setDragEnd] = React.useState(null);
  const [overrides, setOverrides] = React.useState({}); // { 'dlx:5': { rate, closed } }
  const [showBulkSheet, setShowBulkSheet] = React.useState(null); // 'rate' | 'block' | 'open' | null
  ...
  // Generate 30 days starting from May 4, 2026
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '14px', paddingBottom: 130 }}>
        ...
        <Card padding={10}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
            {/* Leading blanks: May 4 2026 is Monday, so 0 blanks */}
            {days.map(d => {
              const i = d.idx;
              ...
              return (
                <div
                  ...
                  onClick={(e) => { if (dragStart == null || dragStart === dragEnd) toggleCell(i); }}
                  style={{
                ...
                >
                  <div className="tnum" style={{
                    ...
                    color: d.isWknd ? T.primary : T.ink, lineHeight: 1,
                  }}>{d.dom}</div>
                  {closed ? (
...
function DiscountForm({ t, onCancel, onSave }) {
  const [d, setD] = React.useState({ name: '', code: '', off: 10, type: '%', minNights: 1, validTill: '31 Dec' });
  return (
    <div>
