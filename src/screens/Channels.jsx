...
export default function Channels({ go, t }) {
  ...
  const [copyTargets, setCopyTargets] = useState({ mmt: true, goibibo: true, booking: true, agoda: false });
  ...
  const [customRange, setCustomRange] = useState({ from: '04 May', to: '03 Jun' });
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 14, paddingBottom: 100 }}>
        ...
        <Card padding={0} style={{ marginBottom: 14, overflow: 'hidden', borderColor: T.primary, borderWidth: 1.5 }}>
          ...
          <div style={{ borderTop: `1px solid ${T.borderSoft}`, background: T.bgSoft }}>
            ...
            {channels.filter(c => c.status !== 'connect').map((c, i) => {
              ...
              return (
                <div key={c.id} style={{ padding: '10px 14px', borderTop: i > 0 ? `1px solid ${T.borderSoft}` : 'none', background: T.card }}>
        ...
        {channels.filter(c => c.status !== 'connect').map(c => (
          <Card key={c.id} style={{ marginBottom: 8 }} padding={12}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: 9, background: c.color, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 13, fontWeight: 700 }}>{c.name[0]}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <Card padding={0} style={{ marginTop: 14, overflow: 'hidden' }}>
          ...
          {[
            ...
            <div key={i} style={{ padding: '9px 14px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: i < arr.length - 1 ? `1px solid ${T.borderSoft}` : 'none' }}>
